# attack_streamlit.py
# 伊安珊夜魂值加攻模拟器 - 基于Streamlit的交互式可视化工具
# 核心功能模块：夜魂值动力学模拟与攻击力加成可视化系统
        ...
        if d > 0:
            ...
            for int_sec in range(int(last_time) + 1, int(t) + 1):
                ...
                if current_value > 54:
                    ...
                    current_value = 54 + (overflow - stored_overflow) # 当前值修正
...
def nightsoul2atk(x, d, atk):
    ...
    # 数据完整性校验
    if total_nightsoul is None or np.isnan(total_nightsoul).any():
        st.error("夜魂值包含异常数据")
        return np.zeros_like(x, dtype=np.float64)
...
with col_control:
    ...
    selected_level = st.selectbox(
        "元素爆发等级",
        options=levels,
        format_func=lambda x: f"{int(x)}级（上限{LEVEL_MAP[x]}）",
        index=levels.index(13),
        key='level_selector'
    )
    st.session_state.max_level = int(selected_level)
    ...
    with col_metric1:
        st.metric("夜魂值", f"{y_ns[idx]:.1f}" if len(y_ns) > 0 else "N/A") 
    with col_metric2:
